...
const sumFileData = function (filePath1, filePath2, callback) {
  ...
  fs.readFile(filePath1, (err, dataFile1) => {
    if (err) return callback(err, null);

    fs.readFile(filePath2, (err, dataFile2) => {
      if (err) return callback(err, null);

      callback(null, Number(dataFile1) + Number(dataFile2));
    });
  });
};

sumFileData('./24.txt', './42.txt', (err, response) => console.log(response));
